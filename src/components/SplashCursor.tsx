...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const context = getWebGLContext(canvasEl);
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let context = canvasEl.getContext('webgl2', params) as WebGL2RenderingContext | null;
      ...
      if (isWebGL2) {
        formatRGBA = getSupportedFormat(
          context,
          (context as WebGL2RenderingContext).RGBA16F,
          context.RGBA,
          halfFloatTexType
        );
        formatRG = getSupportedFormat(
          context,
          (context as WebGL2RenderingContext).RG16F,
          (context as WebGL2RenderingContext).RG,
          halfFloatTexType
        );
        formatR = getSupportedFormat(
          context,
          (context as WebGL2RenderingContext).R16F,
          (context as WebGL2RenderingContext).RED,
          halfFloatTexType
        );
      } else {
    ...
    const vorticityShader = compileShader(
      ...
      void main () {
          ...
          vec2 velocityTex = texture2D(uVelocity, vUv).xy;
          velocityTex += force * dt;
          velocityTex = min(max(velocityTex, -1000.0), 1000.0);
          gl_FragColor = vec4(velocityTex, 0.0, 1.0);
      }
    `
    );
    ...
    const blit = (() => {
      ...
      if (!buffer || !elemBuffer) {
        return (_target: FBO | null, _doClear = false) => undefined;
      }
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: number, param: number): FBO {
      ...
      if (!texture || !fbo) {
        throw new Error('Failed to create framebuffer objects for SplashCursor');
      }
  ...
  return (
    <div className="fixed left-0 top-0 z-50 h-full w-full pointer-events-none">
